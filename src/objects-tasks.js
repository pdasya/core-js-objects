 ...
 *    shallowCopy({}) => {}
 */
function shallowCopy(inputObject) {
  const copyObject = {};
  return Object.assign(copyObject, inputObject);
}
...
function mergeObjects(objectsArray) {
  ...
  objectsArray.forEach((object) => {
    Object.entries(object).forEach(([key, value]) => {
      resultObject[key] = (resultObject[key] ? resultObject[key] : 0) + value;
    });
  });
  return resultObject;
}
 ...
 *
 */
function removeProperties(inputObject, inputKeys) {
...
function compareObjects(inputObject1, inputObject2) {
  const keys1 = Object.keys(inputObject1);
  const keys2 = Object.keys(inputObject2);
  let result = true;
...
function fromJSON(proto, json) {
  ...
  return Object.assign(object, JSON.parse(json));
}
